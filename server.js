	...
	const data = hospitalData
		...
		.map((val) => ({
			...
			doctors: val.doctors.filter(
				(val2) => val2.type.toLowerCase() === doctorType.replace(REG.UNDERSCORE, " ")
			),
		}));
...
app.get("/medicine/:medType", (req, res) => {
	...
	const data2 = concernBasedCategories.filter((val) => val.id === Number(mid));
...
app.get("/medicine/:medType/:id", (req, res) => {
	...
	console.log("🚀 ~ file: server.js ~ line 281 ~ app.get ~ medicineId", medicineId);

	const data = medicineProductData.filter((val) => val.id === Number(medicineId));
...
app.post("/verifyOTP", async (req, res) => {
	...
	if (newCalculatedHash === hashValue) {
		...
		return res.status(200).send({ verification: false, msg: `Incorrect OTP` });
	}
});
...
app.post("/refresh", (req, res) => {
	...
	if (!refreshToken) return res.status(403).send({ msg: `refresh token not found, Pls log in` });
	...
	jwt.verify(refreshToken, JWT_REFRESH_TOKEN, (err, phone) => {
		if (!err) {
			...
			return res
				...
				.send({ previousSessionExpiry: true, success: true, phone: phone });
		} else {
